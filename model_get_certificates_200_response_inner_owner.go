/*
Nginx Proxy Manager API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.12.2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package nginxproxymanager

import (
	"encoding/json"
	"fmt"
)

// checks if the GetCertificates200ResponseInnerOwner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetCertificates200ResponseInnerOwner{}

// GetCertificates200ResponseInnerOwner User object
type GetCertificates200ResponseInnerOwner struct {
	// User ID
	Id int64 `json:"id"`
	// Created Date
	CreatedOn string `json:"created_on"`
	// Modified Date
	ModifiedOn string `json:"modified_on"`
	// Is user Disabled
	IsDisabled bool `json:"is_disabled"`
	// Email
	Email string `json:"email"`
	// Name
	Name string `json:"name"`
	// Nickname
	Nickname string `json:"nickname"`
	// Gravatar URL based on email, without scheme
	Avatar string `json:"avatar"`
	// Roles applied
	Roles []string `json:"roles"`
	AdditionalProperties map[string]interface{}
}

type _GetCertificates200ResponseInnerOwner GetCertificates200ResponseInnerOwner

// NewGetCertificates200ResponseInnerOwner instantiates a new GetCertificates200ResponseInnerOwner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetCertificates200ResponseInnerOwner(id int64, createdOn string, modifiedOn string, isDisabled bool, email string, name string, nickname string, avatar string, roles []string) *GetCertificates200ResponseInnerOwner {
	this := GetCertificates200ResponseInnerOwner{}
	this.Id = id
	this.CreatedOn = createdOn
	this.ModifiedOn = modifiedOn
	this.IsDisabled = isDisabled
	this.Email = email
	this.Name = name
	this.Nickname = nickname
	this.Avatar = avatar
	this.Roles = roles
	return &this
}

// NewGetCertificates200ResponseInnerOwnerWithDefaults instantiates a new GetCertificates200ResponseInnerOwner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetCertificates200ResponseInnerOwnerWithDefaults() *GetCertificates200ResponseInnerOwner {
	this := GetCertificates200ResponseInnerOwner{}
	return &this
}

// GetId returns the Id field value
func (o *GetCertificates200ResponseInnerOwner) GetId() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetIdOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *GetCertificates200ResponseInnerOwner) SetId(v int64) {
	o.Id = v
}

// GetCreatedOn returns the CreatedOn field value
func (o *GetCertificates200ResponseInnerOwner) GetCreatedOn() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.CreatedOn
}

// GetCreatedOnOk returns a tuple with the CreatedOn field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetCreatedOnOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedOn, true
}

// SetCreatedOn sets field value
func (o *GetCertificates200ResponseInnerOwner) SetCreatedOn(v string) {
	o.CreatedOn = v
}

// GetModifiedOn returns the ModifiedOn field value
func (o *GetCertificates200ResponseInnerOwner) GetModifiedOn() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ModifiedOn
}

// GetModifiedOnOk returns a tuple with the ModifiedOn field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetModifiedOnOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ModifiedOn, true
}

// SetModifiedOn sets field value
func (o *GetCertificates200ResponseInnerOwner) SetModifiedOn(v string) {
	o.ModifiedOn = v
}

// GetIsDisabled returns the IsDisabled field value
func (o *GetCertificates200ResponseInnerOwner) GetIsDisabled() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IsDisabled
}

// GetIsDisabledOk returns a tuple with the IsDisabled field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetIsDisabledOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IsDisabled, true
}

// SetIsDisabled sets field value
func (o *GetCertificates200ResponseInnerOwner) SetIsDisabled(v bool) {
	o.IsDisabled = v
}

// GetEmail returns the Email field value
func (o *GetCertificates200ResponseInnerOwner) GetEmail() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Email
}

// GetEmailOk returns a tuple with the Email field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetEmailOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Email, true
}

// SetEmail sets field value
func (o *GetCertificates200ResponseInnerOwner) SetEmail(v string) {
	o.Email = v
}

// GetName returns the Name field value
func (o *GetCertificates200ResponseInnerOwner) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *GetCertificates200ResponseInnerOwner) SetName(v string) {
	o.Name = v
}

// GetNickname returns the Nickname field value
func (o *GetCertificates200ResponseInnerOwner) GetNickname() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Nickname
}

// GetNicknameOk returns a tuple with the Nickname field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetNicknameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Nickname, true
}

// SetNickname sets field value
func (o *GetCertificates200ResponseInnerOwner) SetNickname(v string) {
	o.Nickname = v
}

// GetAvatar returns the Avatar field value
func (o *GetCertificates200ResponseInnerOwner) GetAvatar() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Avatar
}

// GetAvatarOk returns a tuple with the Avatar field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetAvatarOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Avatar, true
}

// SetAvatar sets field value
func (o *GetCertificates200ResponseInnerOwner) SetAvatar(v string) {
	o.Avatar = v
}

// GetRoles returns the Roles field value
func (o *GetCertificates200ResponseInnerOwner) GetRoles() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Roles
}

// GetRolesOk returns a tuple with the Roles field value
// and a boolean to check if the value has been set.
func (o *GetCertificates200ResponseInnerOwner) GetRolesOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Roles, true
}

// SetRoles sets field value
func (o *GetCertificates200ResponseInnerOwner) SetRoles(v []string) {
	o.Roles = v
}

func (o GetCertificates200ResponseInnerOwner) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetCertificates200ResponseInnerOwner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["created_on"] = o.CreatedOn
	toSerialize["modified_on"] = o.ModifiedOn
	toSerialize["is_disabled"] = o.IsDisabled
	toSerialize["email"] = o.Email
	toSerialize["name"] = o.Name
	toSerialize["nickname"] = o.Nickname
	toSerialize["avatar"] = o.Avatar
	toSerialize["roles"] = o.Roles

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *GetCertificates200ResponseInnerOwner) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"created_on",
		"modified_on",
		"is_disabled",
		"email",
		"name",
		"nickname",
		"avatar",
		"roles",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetCertificates200ResponseInnerOwner := _GetCertificates200ResponseInnerOwner{}

	err = json.Unmarshal(data, &varGetCertificates200ResponseInnerOwner)

	if err != nil {
		return err
	}

	*o = GetCertificates200ResponseInnerOwner(varGetCertificates200ResponseInnerOwner)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "created_on")
		delete(additionalProperties, "modified_on")
		delete(additionalProperties, "is_disabled")
		delete(additionalProperties, "email")
		delete(additionalProperties, "name")
		delete(additionalProperties, "nickname")
		delete(additionalProperties, "avatar")
		delete(additionalProperties, "roles")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableGetCertificates200ResponseInnerOwner struct {
	value *GetCertificates200ResponseInnerOwner
	isSet bool
}

func (v NullableGetCertificates200ResponseInnerOwner) Get() *GetCertificates200ResponseInnerOwner {
	return v.value
}

func (v *NullableGetCertificates200ResponseInnerOwner) Set(val *GetCertificates200ResponseInnerOwner) {
	v.value = val
	v.isSet = true
}

func (v NullableGetCertificates200ResponseInnerOwner) IsSet() bool {
	return v.isSet
}

func (v *NullableGetCertificates200ResponseInnerOwner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetCertificates200ResponseInnerOwner(val *GetCertificates200ResponseInnerOwner) *NullableGetCertificates200ResponseInnerOwner {
	return &NullableGetCertificates200ResponseInnerOwner{value: val, isSet: true}
}

func (v NullableGetCertificates200ResponseInnerOwner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetCertificates200ResponseInnerOwner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


